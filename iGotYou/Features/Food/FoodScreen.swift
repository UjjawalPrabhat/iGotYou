...
struct FoodScreen: View {
    ...
    private var allCarts: some View {
        ZStack(alignment: .bottom) {
            ...
            .padding(.bottom, IGY.S.dockBottom)
    ...
    private var pinnedHeader: some View {
        ...
        .background {
            Rectangle()
                .fill(IGY.C.surface.opacity(0.94))
                        ...
                        VStack(alignment: .leading, spacing: 0) {
                            dish.art.placeholder.caption("dish photo")
                                ...
                                .clipShape(RoundedRectangle(cornerRadius: IGY.R.control,
                                                            style: .continuous))
                                .overlay(
                                    RoundedRectangle(cornerRadius: IGY.R.control,
                                                     style: .continuous)
                                        .strokeBorder(IGY.C.hairline, lineWidth: 1)
                                )
                            Text(dish.name).textRole(.rowTitleSm).padding(.top, 8)
                            Text(dish.price).textRole(.caption, IGY.C.inkMuted)
                        }
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                HStack(spacing: 3) {
                    Text("View menu").textRole(.caption, IGY.C.inkMuted)
            ...
            Button {} label: {
                ...
                .padding(.horizontal, 16)
                .padding(.vertical, 9)
                .background(IGY.C.brandDeep, in: Capsule())
            }
            .buttonStyle(.plain)
...
private struct AllCartsPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(total).textRole(.caption, IGY.C.inkMuted)
                }
                Spacer(minLength: 8)
                Button(action: onClearAll) {
                    Text("Clear all").textRole(.label, IGY.C.alert)
                }
                .buttonStyle(.plain)
            }
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .glassEffect(.regular,
                     in: .rect(cornerRadius: IGY.R.sheet, style: .continuous))
        .padding(.horizontal, IGY.S.dockInset)
...
private struct CartRow: View {
    ...
    var body: some View {
        let shape = RoundedRectangle(cornerRadius: IGY.R.list, style: .continuous)
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(cart.restaurant).textRole(.rowTitleSm).lineLimit(1)
                Text(cart.summary)
                    .textRole(.caption, cart.isAvailable ? IGY.C.inkMuted : IGY.C.alert)
                    .lineLimit(1)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
        ...
        .background {
            shape
                .fill(IGY.C.alert.opacity(min(-dragX / threshold, 1) * 0.9))
...
private struct RestaurantCard: View {
    ...
    var body: some View {
        Button {} label: {
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text(restaurant.cuisine)
                        .textRole(.bodySm, IGY.C.inkMuted)
                        .padding(.top, 3)
            ...
            .clipShape(RoundedRectangle(cornerRadius: IGY.R.card, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: IGY.R.card, style: .continuous)
                    .strokeBorder(IGY.C.hairline, lineWidth: 1)
            )
        }
