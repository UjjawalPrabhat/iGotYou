    ...
    private var sheet: some View {
        VStack(spacing: 0) {
            ...
            Text("CHOOSE A VEHICLE")
                .textRole(.sheetLabel, IGY.C.inkMuted)
        ...
        HStack(spacing: 12) {
            ...
            HStack(spacing: 9) {
                RoundedRectangle(cornerRadius: 4, style: .continuous)
                    .strokeBorder(IGY.C.violet, lineWidth: 2)
                    .frame(width: 22, height: 16)
                VStack(alignment: .leading, spacing: 0) {
                    Text("iGotYou Wallet").textRole(.label)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 7) {
                        ...
                        if let badge = vehicle.badge {
                            Text(badge)
                                .textRole(.badge, IGY.C.brandDeep)
                                .padding(.horizontal, 7)
                                .padding(.vertical, 3)
                                .background(IGY.C.brandTint,
                                            in: RoundedRectangle(cornerRadius: 6,
                                                                 style: .continuous))
                        }
                    }
                    Text(vehicle.detail).textRole(.bodySm, IGY.C.inkMuted)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            if showsRoute {
                ...
                ctx.stroke(route, with: .color(IGY.C.brand),
        ...
        .overlay(alignment: .topLeading) {
            ...
            Circle()
                ...
                .overlay(Circle().strokeBorder(IGY.C.ink.opacity(0.14), lineWidth: 4)
        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                if !detail.isEmpty {
                    ...
                    Text(detail).textRole(.caption, IGY.C.inkMuted).lineLimit(1)
                }
            }
        }
