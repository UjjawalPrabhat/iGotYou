...
struct RideIcon: View {
    var size: CGFloat = 48
    var palette: IconPalette = .init(primary: IGY.C.brandBright,
                                     secondary: Color(hex: 0xF5F8F7),
                                     tertiary: IGY.C.gold)
    var ink: Color = IGY.C.iconInk
...
struct FoodIcon: View {
    var size: CGFloat = 48
    var palette: IconPalette = .init(primary: IGY.C.coral,
                                     secondary: IGY.C.gold,
                                     tertiary: Color(hex: 0xF5F8F7),
                                     quaternary: IGY.C.coral)
    var ink: Color = IGY.C.iconInk
    var band: IconBand? = nil
...
struct SendIcon: View {
    var size: CGFloat = 44
    var palette: IconPalette = .init(primary: IGY.C.blueLight,
                                     secondary: IGY.C.blueMid,
                                     tertiary: IGY.C.blueDeep)
    var ink: Color = IGY.C.iconInk
    var band: IconBand? = nil
...
struct RewardsIcon: View {
    var size: CGFloat = 44
    var palette: IconPalette = .init(primary: IGY.C.goldShade,
                                     secondary: IGY.C.gold,
                                     tertiary: IGY.C.coral)
    var ink: Color = IGY.C.iconInk
    var band: IconBand? = nil
...
/// A statement: a sheet with a curled foot, two ruled lines and a currency
/// mark.
///
/// Drawn rather than taken from the illustration set, because the service
/// cards are not the only place a service icon appears — search results show
/// one at 18pt and activity rows at 28pt, and an illustration at those sizes
/// is mush. The set has to stay complete at every size the app asks for.
struct BillsIcon: View {
    var size: CGFloat = 44
    var palette: IconPalette = .init(primary: Color(hex: 0xFFF6E6),
                                     secondary: IGY.C.goldShade,
                                     tertiary: IGY.C.coral)
    var ink: Color = IGY.C.iconInk
    var band: IconBand? = nil
    ...
    var body: some View {
        ...
        ZStack {
            // Sheet, with the bottom edge curling up at the left.
            Stroked(path: Path { p in
                p.move(to: g.p(11, 9))
                p.addLine(to: g.p(30, 9))
                p.addLine(to: g.p(30, 29))
                p.addQuadCurve(to: g.p(24, 31), control: g.p(27, 31))
                p.addLine(to: g.p(13, 31))
                p.addQuadCurve(to: g.p(11, 28), control: g.p(11, 30))
                p.closeSubpath()
            }, fill: palette.primary, ink: ink, width: w)

            // Two ruled lines.
            Stroked(path: Path { p in
                p.move(to: g.p(20, 16)); p.addLine(to: g.p(26, 16))
                p.move(to: g.p(15, 22)); p.addLine(to: g.p(26, 22))
            }, fill: nil, ink: ink, width: w)

            // The amount mark.
            Stroked(path: Path { p in
                p.move(to: g.p(15, 19))
                p.addLine(to: g.p(15, 13))
                p.addLine(to: g.p(17, 13))
                p.addQuadCurve(to: g.p(17, 16.5), control: g.p(18.6, 14.75))
                p.addLine(to: g.p(15, 16.5))
            }, fill: nil, ink: palette.tertiary, width: w)
        }
        .frame(width: size, height: size)
    }
}
...
struct CarIcon: View {
    var size: CGFloat = 42
    var palette: IconPalette = .init(primary: IGY.C.blueMid,
                                     secondary: Color(hex: 0xF5F8F7))
    var ink: Color = IGY.C.iconInk
    var band: IconBand? = nil
...
struct IconPalette {
    ...
    static func ride(onCard: Bool = true) -> IconPalette {
        .init(primary: IGY.C.brandBright,
    ...
    static let deliveryInFlight = IconPalette(primary: IGY.C.brandBright,
                                              secondary: .white,
                                              tertiary: .white,
                                              quaternary: IGY.C.brandBright)
}
