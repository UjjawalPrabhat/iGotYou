...
/// The four doors on Home.
///
/// Rewards used to be the fourth and isn't any more. It's reachable from
/// Wallet, which has a tab — and the scope rule the whole grid is built on is
/// "one entry point each: nothing with a tab gets a door". Rewards having both
/// was the one place that rule was broken. Credit & bills takes the slot
/// because nothing else reaches it.
enum Service: String, CaseIterable, Identifiable, Hashable {
    case ride, food, send, bills
    ...
    /// The illustration, for the Home doors only.
    ///
    /// These are full-colour scenes, and they stop working somewhere around
    /// 40pt — so everywhere smaller (search results at 18, activity rows at 28,
    /// the service header at 28) keeps the drawn set below. Two representations
    /// of the same service is a cost worth paying: the alternative is either
    /// illegible thumbnails or flat glyphs on the one screen with room for
    /// something better.
    var artwork: String {
        switch self {
        case .ride:  "grab_ride"
        case .food:  "grab_food"
        case .send:  "grab_express"
        case .bills: "grab_bills"
        }
    }
