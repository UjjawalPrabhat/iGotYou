...
// apply `.irobGlass` only to something that floats, and never inside another
...
struct HairlineCard: ViewModifier {
    ...
    var fill: Color = IRob.C.card
    var shadow: IRob.Shadow? = nil

    func body(content: Content) -> some View {
        let shape = RoundedRectangle(cornerRadius: radius, style: .continuous)
        return content
            .background(fill, in: shape)
            .overlay(shape.strokeBorder(IRob.C.hairline, lineWidth: 1))
            .irobShadow(shadow)
    }
}

extension View {
    func hairlineCard(radius: CGFloat = IRob.R.list,
                      fill: Color = IRob.C.card,
                      shadow: IRob.Shadow? = nil) -> some View {
        modifier(HairlineCard(radius: radius, fill: fill, shadow: shadow))
    }

    /// A card that also lifts — the Home service doors.
    func liftedCard(radius: CGFloat = IRob.R.feature) -> some View {
    ...
    func selectedCard(radius: CGFloat = IRob.R.cardTight, selected: Bool) -> some View {
        let shape = RoundedRectangle(cornerRadius: radius, style: .continuous)
        return self
            .background(IRob.C.card, in: shape)
            .overlay(
                shape.strokeBorder(selected ? IRob.C.brandDeep : IRob.C.hairline,
...
extension View {
    ...
    func irobGlass(radius: CGFloat = IRob.R.full,
                   shadow: IRob.Shadow = .dock) -> some View {
...
struct StripePlaceholder: View {
    ...
    static let mint  = StripePlaceholder(a: IRob.C.brandTint,  b: IRob.C.washGreen)
    static let coral = StripePlaceholder(a: IRob.C.coralTint,  b: IRob.C.coralWash)
    static let gold  = StripePlaceholder(a: IRob.C.goldTint,   b: IRob.C.goldWash)
    static let blue  = StripePlaceholder(a: IRob.C.blueTint,   b: IRob.C.blueWash)
    static let lilac = StripePlaceholder(a: IRob.C.violetTint, b: Color(hex: 0xF6F4FD))
