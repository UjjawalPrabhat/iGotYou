...
struct WalletGlyph: View {
    var body: some View {
        RoundedRectangle(cornerRadius: 5, style: .continuous)
            .strokeBorder(IRob.C.inkSecondary, lineWidth: 2)
...
struct ActivityGlyph: View {
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            RoundedRectangle(cornerRadius: 1).fill(IRob.C.inkSecondary)
...
struct BellGlyph: View {
    var size: CGFloat = 16
    var color: Color = IRob.C.inkSecondary
...
struct AvatarGlyph: View {
    ...
    var body: some View {
        Circle()
            .fill(onDark ? Color.white.opacity(0.22) : IRob.C.brandTint)
            .overlay(
                Circle().strokeBorder(onDark ? .white.opacity(0.55)
                                             : IRob.C.brandTintEdge,
                                      lineWidth: 1)
            )
            .frame(width: size, height: size)
    }
...
struct CountBadge: View {
    ...
    var body: some View {
        Text("\(count)")
            ...
            .background(IRob.C.alert, in: Capsule())
