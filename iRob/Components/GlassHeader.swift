...
struct GlassHeader: View {
    ...
    private var titleInk: Color { onDark ? IRob.C.onBrand : IRob.C.ink }
    private var subInk: Color { onDark ? IRob.C.onBrandMuted : IRob.C.inkMuted }
    ...
    private func picker(label: String, value: String) -> some View {
        Button(action: onLocation) {
            VStack(alignment: .leading, spacing: 1) {
                ...
                HStack(spacing: 5) {
                    PinGlyph(color: onDark ? IRob.C.onBrand : IRob.C.brand)
    ...
    private func bell(badge: Int) -> some View {
        Button(action: onBell) {
            ZStack(alignment: .topTrailing) {
                ...
                BellGlyph(color: onDark ? IRob.C.onBrand : IRob.C.inkSecondary)
                    .frame(width: 36, height: 36)
                if badge > 0 {
    ...
    var body: some View {
        ...
        .overlay(alignment: .bottom) {
            if onDark == false && reduceTransparency {
                Rectangle().fill(IRob.C.hairline).frame(height: 0.5)
            }
        }
