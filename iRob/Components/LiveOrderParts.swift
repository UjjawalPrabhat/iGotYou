...
struct PulseBadge: View {
    ...
    var body: some View {
        ZStack {
            Circle().fill(IRob.C.brandTint)

            if pulsing && !reduceMotion {
                Circle()
                    .strokeBorder(IRob.C.brandBright, lineWidth: 2)
...
struct ProgressSegments: View {
    let filled: Int
    var total: Int = 4
    var trackColor: Color = IRob.C.ink.opacity(0.14)

    var body: some View {
        HStack(spacing: 3) {
            ForEach(0..<total, id: \.self) { i in
                Capsule()
                    .fill(i < filled ? IRob.C.brand : trackColor)
                    .frame(height: 4)
            }
        }
    ...
    var body: some View {
        ...
        HStack(spacing: dot + 1) {
            ForEach(0..<count, id: \.self) { i in
                Circle()
                    .fill(i == index ? IRob.C.brand : IRob.C.ink.opacity(0.22))
