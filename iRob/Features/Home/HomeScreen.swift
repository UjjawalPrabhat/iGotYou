...
struct HomeScreen: View {
    ...
    /// thumbnail, and an iRob that reads as a different company fails the brief
    /// before any of the structure gets looked at.
    ///
    /// The greeting sits *inside* it in white, so the question the screen is
    /// built around ("What can iRob help with?") lands on the brand rather than
    ...
    private var greetingBand: some View {
        VStack(alignment: .leading, spacing: 3) {
            Spacer(minLength: 0)
            Text(Mock.greeting).textRole(.greeting, IRob.C.onBrand)
            Text(Mock.greetingAsk).textRole(.body, IRob.C.onBrandMuted)
        }
        ...
        .background(
            LinearGradient(
                ...
                stops: [
                    .init(color: IRob.C.brandDark, location: 0),
                    .init(color: IRob.C.brandDark, location: 0.52),
                    .init(color: IRob.C.brandMid,  location: 1),
                ],
    ...
    private var serviceGrid: some View {
        ...
        // and Send/Rewards 132pt, and a grid would force both rows to the
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(spacing: 12) {
                ...
                ServiceCard(service: .rewards) { app.presentedService = .rewards }
            }
        }
    }
