                ...
                TextField("Where to?", text: $query)
                    ...
                    .background(IRob.C.card,
                                in: RoundedRectangle(cornerRadius: IRob.R.control,
                                                     style: .continuous))
                    .overlay(
                        RoundedRectangle(cornerRadius: IRob.R.control, style: .continuous)
                            .strokeBorder(fieldFocused ? IRob.C.brandDeep : IRob.C.hairline,
    ...
    private var tabs: some View {
        HStack(spacing: 8) {
            ForEach(Tab.allCases) { t in
                Button { tab = t } label: {
                    Text(t.rawValue)
                        .textRole(.label, tab == t ? IRob.C.brandDeep : IRob.C.inkMuted)
                        .padding(.horizontal, 16)
                        .padding(.vertical, 9)
                        .background(tab == t ? IRob.C.brandTint : .clear, in: Capsule())
                }
                .buttonStyle(.plain)
            ...
            LazyVStack(spacing: 0) {
                if results.isEmpty {
                    Text("Nothing for “\(query)”")
                        .textRole(.rowTitle, IRob.C.inkMuted)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .padding(.horizontal, IRob.S.gutter)
                        .padding(.top, 28)
                } else {
                    ForEach(Array(results.enumerated()), id: \.element.id) { i, place in
                        if i > 0 { RowDivider(inset: IRob.S.gutter + 38) }
...
private struct PickupScreen: View {
    ...
    private var searchBar: some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 10) {
                Circle().fill(IRob.C.brand).frame(width: 10, height: 10)
                Text("Pick up at?")
                    .textRole(.rowTitle, IRob.C.inkMuted)
                    .lineLimit(1)
                Spacer(minLength: 0)
            ...
            .frame(height: 48)
            .glassEffect(.regular,
                         in: .rect(cornerRadius: IRob.R.control, style: .continuous))
        }
        ...
        ScrollView {
            LazyVStack(spacing: 0) {
                ForEach(Array(Mock.pickupPoints.enumerated()), id: \.element.id) { i, point in
                    VStack(spacing: 0) {
                        if i > 0 {
                            RowDivider(inset: IRob.S.gutter + 33)
                        }
            ...
            HStack(spacing: 10) {
                if editingNote {
                    ...
                    Text(note.isEmpty ? Mock.pickupNotePrompt : note)
                        .textRole(.bodySm, note.isEmpty ? IRob.C.brandDeep : IRob.C.ink)
                        .lineLimit(1)
                }
                Spacer(minLength: 0)
                Image(systemName: editingNote ? "checkmark.circle.fill" : "plus.circle.fill")
                    .font(.system(size: 18))
                    .foregroundStyle(IRob.C.brandDeep)
            }
            .padding(.vertical, 14)
            .contentShape(.rect)
        }
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ForEach(Array(Mock.pickupPoints.enumerated()), id: \.element.id) { i, point in
                ...
                PinGlyph(size: 13, color: IRob.C.brandDeep, filled: true)
                    .opacity(isOn ? 0 : 1)
                    .offset(x: spot.x, y: spot.y)

                Text(point.pinLabel)
                    .textRole(.chip, isOn ? IRob.C.ink : IRob.C.inkSecondary)
                    .fixedSize()
            ...
            PinGlyph(size: 26, color: IRob.C.brand, filled: true)
                .irobShadow(.card)
...
private struct RouteRail: View {
    var body: some View {
        VStack(spacing: 5) {
            Circle()
                .strokeBorder(IRob.C.brand, lineWidth: 4)
                .frame(width: 14, height: 14)

            ForEach(0..<3, id: \.self) { _ in
                Circle().fill(IRob.C.hairline).frame(width: 3, height: 3)
            }

            PinGlyph(size: 14, color: IRob.C.coral, filled: true)
        }
        .frame(width: 16)
        .accessibilityHidden(true)
    }
...
private struct SegmentedPill: View {
    ...
    var body: some View {
        HStack(spacing: 2) {
            ForEach(RideTiming.allCases) { option in
                ...
                Button {
                    ...
                    .foregroundStyle(isOn ? IRob.C.ink : IRob.C.inkMuted)
                    .padding(.horizontal, 18)
                    .padding(.vertical, 9)
                    .background {
                        if isOn {
                            Capsule().fill(IRob.C.card).irobShadow(.card)
                        }
                    }
                }
                .buttonStyle(.plain)
        ...
        .padding(3)
        .background(IRob.C.surfaceMuted, in: Capsule())
    }
}
...
private struct PlaceRow: View {
    ...
    private var tint: Color {
        switch place.kind {
        case .saved: IRob.C.coral
        case .suggested: IRob.C.brandDeep
        default: IRob.C.inkMuted
        }
    }
}
        ...
        Button(action: onTap) {
            HStack(spacing: 13) {
                Image(systemName: selected ? "largecircle.fill.circle" : "circle")
                    .font(.system(size: 20))
                    .foregroundStyle(selected ? IRob.C.brandDeep : IRob.C.hairline)
            ...
            // the same thing.
            .background(selected ? IRob.C.brandTint : .clear)
            .contentShape(.rect)
        }
        .buttonStyle(.plain)
        .accessibilityElement(children: .combine)
